...
7. The receiver deduplicates the UUID, relays it to other peers, materializes file bundles, normalizes each representation's type name to the target platform's convention, and atomically publishes all safe representations. X11 selection atoms and MIME types become Uniform Type Identifiers for `NSPasteboard`, which would otherwise reject them and leave the pasteboard empty; several incoming names may collapse to a single pasteboard entry, and names the platform cannot represent are dropped. Linux backends accept any name and receive them unchanged. On macOS, a single image file is published as one pasteboard item containing both its native file URL and image data, making the same value pasteable in Finder and image-aware terminal or chat inputs. Multi-file selections remain native `NSURL` objects for Finder.
